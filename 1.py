...
class SegmentationDataset(Dataset):
    def __init__(self, images, masks, augment=False, cutmix_prob=0.5):
        """
        Args:
            images: List of numpy arrays (C, H, W) - raw Sentinel values
    ...
    def apply_cutmix(self, image, mask, current_idx):
        """
        CutMix: Replace a random rectangular region with patch from another image.
        Works on raw Sentinel values (before normalization) to preserve bit depth.
        """
        ...
        # Lambda from Beta distribution determines box size
        ...
        # Bounding box coordinates (clipped to image bounds)
    ...
    def __getitem__(self, idx):
        # Copy to avoid modifying original list data
        image = self.images[idx].copy()
        mask = self.masks[idx].copy()

        if self.augment:
            # 1. CutMix (before normalization to preserve raw values)
        ...
        # Normalize AFTER augmentations (Sentinel-2 data)
        image = image / 10000.0
...
if __name__ == '__main__':
    ...
    print(f"MSI dtype: {msi_arr.dtype}")
    print(f"MSI value range: [{msi_arr.min()}, {msi_arr.max()}]")
    ...
    # Class distribution
    class_counts = np.bincount(label_arr_resampled.astype(int).flatten())
    total_pixels = class_counts.sum()
    print(f"Class distribution: {class_counts}")
    print(f"Class 0 (background): {class_counts[0]:,} ({100*class_counts[0]/total_pixels:.2f}%)")
    print(f"Class 1 (target):     {class_counts[1]:,} ({100*class_counts[1]/total_pixels:.2f}%)")
    ...
    if len(image_patches) < 5:
        print("Warning: Very few patches extracted. Consider reducing patch_size or stride.")
    ...
    # Create datasets with CutMix augmentation for training
    ...
    print('\nFinal model weights saved to final_model_weights.pth')
    print('Best model saved to best_model.pth')
    
    # Cleanup
    del train_images, val_images, train_masks, val_masks
    ...
    with torch.no_grad():
        ...
        true_positive = np.sum(label_arr_resampled)
        print(f"\n  Ground truth positive pixels: {int(true_positive):,} ({100*true_positive/total_pixels:.2f}%)")
        print(f"  Predicted positive pixels:    {int(pred_positive):,} ({100*pred_positive/total_pixels:.2f}%)")
        
        # Save outputs
        save_raster_tif('prediction.tif', binary_pred.astype(np.uint8), bbox, str(projection))
        print("\nPrediction saved to prediction.tif")
        
        save_raster_tif('probability_map.tif', prob_map.astype(np.float32), bbox, str(projection))
        print("Probability map saved to probability_map.tif")
